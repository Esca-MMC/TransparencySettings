...
{
    public static class HarmonyPatch_FruitTreeTransparency
    {
        ...
        public static void ApplyPatch(HarmonyInstance harmony, IModHelper helper, IMonitor monitor)
        {
            if (!Applied && helper != null && monitor != null) //if NOT already applied AND valid tools were provided
            {
                ...
                Monitor.Log($"Applying Harmony patch \"{nameof(HarmonyPatch_FruitTreeTransparency)}\": postfixing SDV method \"FruitTree.tickUpdate(GameTime, Vector2, GameLocation)\".", LogLevel.Trace);
                harmony.Patch(
                    original: AccessTools.Method(typeof(FruitTree), nameof(FruitTree.tickUpdate), new[] { typeof(GameTime), typeof(Vector2), typeof(GameLocation) }),
                    postfix: new HarmonyMethod(typeof(HarmonyPatch_FruitTreeTransparency), nameof(FruitTree_updateTick))
                );
        ...
        private static void FruitTree_updateTick(FruitTree __instance, Vector2 tileLocation)
        {
            try
            {
                ...
                {
                    if (__instance.growthStage.Value >= 4 && __instance.stump.Value == false) //if this tree is fully grown and NOT cut down
                    {
            ...
            {
                Monitor.LogOnce($"Harmony patch \"{nameof(HarmonyPatch_FruitTreeTransparency)}\" has encountered an error. Custom fruit tree transparency might not work correctly. Full error message: \n{ex.ToString()}", LogLevel.Error);
