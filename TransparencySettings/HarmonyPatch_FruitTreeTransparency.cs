...
namespace TransparencySettings
{
    public static class HarmonyPatch_FruitTreeTransparency
    {
        ...
        private static void FruitTree_updateTick(FruitTree __instance)
        {
            try
            {
                if (ModEntry.Config.TreeSettings.Enable && InputManager.DisableTransparency.Value == false) //if this type of custom transparency is enabled
                {
                    if (__instance.growthStage.Value >= 4 && __instance.stump.Value == false) //if this tree is fully grown and NOT cut down
                    {
                        ...
                        Vector2 treeTile = new Vector2(__instance.Tile.X, __instance.Tile.Y - 2); //adjust toward the center of the tree's sprite
